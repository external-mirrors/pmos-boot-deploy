...
# Create environ.txt file that can be used in U-Boot bootscripts
create_uboot_env() {
	[ "${deviceinfo_generate_uboot_env}" = "true" ] || return 0

	log_arrow "Generating u-boot environment"

	local _cmdline_line=""
	if [ ! "$(get_cmdline)" = " " ]; then
		_cmdline_line="bootargs=$(get_cmdline)"
	fi
	local _dtb_line=""
	if [ -n "$deviceinfo_dtb" ] && [ -n "$(find_dtb "$deviceinfo_dtb")" ]; then
		_dtb_line="devicetree=/$(basename "$deviceinfo_dtb").dtb"
	fi

	cat <<EOF > "$work_dir/environ.txt"
# DO NOT EDIT -- u-boot environment generated by boot-deploy
$_cmdline_line
$_dtb_line
EOF

	additional_files="$additional_files environ.txt"
}
