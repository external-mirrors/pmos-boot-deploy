...
create_bootimg() {
	...
	if [ "${deviceinfo_bootimg_qcdt}" = "true" ]; then
		...
		if [ -n "${deviceinfo_bootimg_qcdt_type}" ]; then
			...
			additional_files="$additional_files $(basename "$_dt_img_out")"
		else
			# The kernel APKBUILD usually installs the dt.img in /boot/.
			# Check whether the file exists or not.
			_dt_img_out="/boot/dt.img"
			if ! [ -e "$_dt_img_out" ]; then
				log "ERROR: File not found: $_dt_img_out, but"
