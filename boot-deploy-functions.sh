...
add_limine() {
	[ "$deviceinfo_generate_limine" = "true" ] || return 0
	log_arrow "limine: adding support"

	cat <<-EOF > "$work_dir/limine.cfg"
		# !!!! Generated by boot-deploy
		...
		COMMENT=$distro_name
		PROTOCOL=linux
		KERNEL_PATH=boot:///$kernel_filename
		KERNEL_CMDLINE=$(get_cmdline)
		MODULE_PATH=boot:///$initfs_filename
	EOF
	# note: this conf is shoved into efi/boot to make it less tempting for
	# users to modify it directly
	additional_files="$additional_files limine.cfg:efi/boot/limine.cfg"

	local _found_efi_app=false
	# looping and including all EFI apps allow supporting different EFI archs
	# on a single CPU arch (e.g. 32-bit EFI on x86_64 CPU), since they can just
	# be installed under /usr/share/limine and no other configuration is
	# necessary
	for f in /usr/share/limine/BOOT*; do
		_found_efi_app=true
		local _efi_app
		_efi_app="$(basename "$f")"
		copy "$f" "$work_dir/$_efi_app"
		additional_files="$additional_files $_efi_app:efi/boot/$_efi_app"
	done

	if ! "$_found_efi_app"; then
		log "ERROR: a limine EFI app was not found in /usr/share/limine"
		exit 1
	fi
}
